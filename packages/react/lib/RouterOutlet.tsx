...
import { RouteMatchData } from '@easyroute/core/dist/types';
...
export const RouterOutlet = (props: PropsWithChildren<RouterOutletProps>) => {
  ...
  if (!router || nestingDepth === undefined) {
    throw new Error(
      '[Easyroute] RouterOutlet: no router instance found. Did you forget to wrap your ' +
  ...
  const changeComponent = useCallback(
    async (
      ...
      const distComponent = isClassComponent ? (
      ...
      if (!transitionData) {
        ...
        componentChangeWithTransition(
          ...
          (c: string) => setTransitionClassName(c),
          (c: any) => setComponent(c),
          router,
          transitionData
        );
      }
  ...
  const matchedSubscribe = useCallback(
    async (matchedRoutes: RouteMatchData[]) => {
      const componentProps = {
        router,
        currentRoute: router.currentRoute
      };

      const currentRoute = matchedRoutes.find(
        (route) => route.nestingDepth === nestingDepth
      );
      if (currentRoute) {
        let componentValue;
        if (name === 'default')
  ...
  return (
    <EasyrouteContext.Provider
      value={{ router, nestingDepth: nestingDepth + 1 }}
    >
